...
def get_columns_for_days(filters: Filters) -> list[dict]:
	total_days = get_days_in_month(filters)
	days = []
	...
	for day in total_days:
		...
		days.append({"label": f'{day.day} {month_dict.get(day.month)}', "fieldtype": "Data", "fieldname": f'{day}', "width": 65})

	return days

def get_days_in_month(filters: Filters) -> int:
	...
	while curr_date <= to_date:
		date_list.append(getdate(curr_date))
...
def get_holiday_status(day: int, holidays: list) -> str:
	...
	if holidays:
		for holiday in holidays:
			if day == holiday.get('holiday_date'):
